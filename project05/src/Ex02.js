...
const Ex02 = () => {
    ...
    A .깊숙히 위치한 컴포넌트에 데이터를 전달해야할 경우 , 여러 컴포넌트에 연달아 Props를 전달해주어야 한다. => 불편하고 실수가 잦음 (Props Drilling)

    So, context로 상태를 전역적으로 관리해줄 것

    **만드는 순서 
    1. Context를 생성해준다. (create context = > expor )

    2. context를 사용할 컴포넌트에 import

    3. 컴포넌트를 Context Provider 로 감싸준다.

    4. 데이터를 받아올 컴포넌트에서 useContext 함수를 사용한다. 
    
    */
    const [choiceColor, setChoiceColor] = useState('red')

  return (
    //value 안에는 우리가 전역적으로 사용할 데이터들을 넣어 줄 것 
